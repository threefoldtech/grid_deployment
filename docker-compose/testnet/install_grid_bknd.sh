...
# Download and extract indexer snapshot for testnet
download_with_retry "rsync://bknd.snapshot.grid.tf:34873/gridsnapshotstest/indexer-testnet-latest.tar.gz" "indexer-testnet-latest.tar.gz"
if [ $? -eq 0 ]; then
  pv indexer-testnet-latest.tar.gz | tar -I pigz -x -C /srv/indexer/
  rm indexer-testnet-latest.tar.gz
fi

# Download and extract processor snapshot for testnet
download_with_retry "rsync://bknd.snapshot.grid.tf:34873/gridsnapshotstest/processor-testnet-latest.tar.gz" "processor-testnet-latest.tar.gz"
if [ $? -eq 0 ]; then
  pv processor-testnet-latest.tar.gz | tar -I pigz -x -C /srv/processor/
